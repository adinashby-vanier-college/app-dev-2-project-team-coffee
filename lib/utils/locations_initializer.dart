...
/// Utility class to initialize locations in Firebase
/// This can be called once to populate the database
class LocationsInitializer {
  final LocationsService _locationsService = LocationsService();

  /// Uploads all 35 base locations to Firebase
  /// Returns the number of locations uploaded
  Future<int> initializeLocations() async {
    try {
      final locations = _getAllLocations();
      await _locationsService.uploadLocations(locations);
      return locations.length;
    } catch (e) {
      throw Exception('Failed to initialize locations: $e');
    }
  }

  /// Checks if locations already exist in Firebase
  Future<bool> locationsExist() async {
    final locations = await _locationsService.getAllLocations();
    return locations.isNotEmpty;
  }

  /// Gets all 35 locations as a list
  List<Map<String, dynamic>> _getAllLocations() {
    // Parse the JSON string from the locations data
    final locationsJsonString = _getLocationsJsonString();
    final List<dynamic> decoded = jsonDecode(locationsJsonString);
    return decoded.map((item) => item as Map<String, dynamic>).toList();
  }

  /// Returns the locations JSON as a string
  String _getLocationsJsonString() {
    // Use raw string to handle dollar signs properly
    return r'''[
  {
    "id": "mtl-mount-royal",
